					...
					[{
						...
						},
						{
							...
							templet: function(d) {
								...
								return appName || d.app; // 如果找不到应用名称则显示应用代码
							}
						},
						{
							field: "comment",
...
// 			/* edit: "text",style: "color:#F581B1" 列可编辑*/
			...
			function Del(table, checkStatus) {
				...
				if (data.length > 0) {
					layer.confirm("确定删除选中的 " + data.length + " 个卡密吗？<br>此操作不可恢复！", {
						icon: 3,
						title: '删除确认',
						btn: ['确定删除','取消']
					}, function() {
						$.ajax({
							...
							success: function(res) {
								if (res.code == 1) {
									if(res.errors) {
										// 如果有部分失败
										layer.alert(res.msg + "<br>详细信息：<br>" + res.errors.join("<br>"), {
											icon: 0
										});
									} else {
										layer.msg(res.msg, {
