...
class EpayCore
{
	...
	public function verifyNotify(){
		if(empty($_GET)) {
			error_log("Empty GET parameters");
			return false;
		}
		...
		error_log("Generated sign: " . $sign);
		error_log("Received sign: " . $_GET['sign']);
		
		// 比较签名
		if($sign === $_GET['sign']){
			error_log("Sign verification successful");
			return true;
		} else {
	...
	private function getSign($param){
		...
		error_log("Sign string before MD5: " . $signstr);
		$sign = md5($signstr);
		error_log("Generated sign: " . $sign);
